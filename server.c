...
void listener_socket_cb(evutil_socket_t sock, short ev, void *arg) {
    ...
    evutil_socket_t connection_socket = accept(sock, (struct sockaddr *) &sin,
                                               &addr_size);
    ...
    if (!cl) {
        ...
        fprintf(stderr,
                "Ignoring connection attempt from %s:%d due to lack of space.\n",
    ...
    struct event *an_event =
            event_new(base, connection_socket, EV_READ | EV_PERSIST,
                      client_socket_cb, (void *) cl);
    if (!an_event) syserr("Error creating event.");
    cl->ev = an_event;
    if (event_add(an_event, NULL) == -1)
        syserr("Error adding an event to a base.");
}
...
int main(int argc, char *argv[]) {
    ...
    struct event *listener_socket_event =
            event_new(base, listener_socket, EV_READ | EV_PERSIST,
                      listener_socket_cb, (void *) base);
